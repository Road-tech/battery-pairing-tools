...
> 截图：区块①「上传电芯数据 & 参数设置」合并了上传与参数填写；区块②「一致性分析」展示已解析电芯数据（4 列含换算容量）、整体统计（中心偏差率+集中率）、质量评估（基于分布形态评级，少数离群不拖低等级）及内阻分布（精确到小数点后 1 位）；区块④「配对计算」的配对清单改为串内标签页，每串一个标签（含达标状态点）+ 末尾「未使用」页，横向滚动切换。
...
工具不再要求用户手动填写「换算容量」。满容量由系统根据电表（分容柜）的**测试三参数**自动换算，换算结果统一用于一致性分析、配对分配与成品容量预估：
...
- **① 电压区间换算**：在 [起始电压 → 截止电压] 区间测得的容量，按电池放电曲线反推到「满充上限 → 最低截止(2.5V)」的满容量。**关键事实**：三元锂 4.2V→3.0V 仅放出约 **90%** 的满容量，3.0V→2.5V 尾段约占 **8%~13%**（依大厂手册与实测：Samsung INR18650-25R 以 0.2C 放到 2.5V 标称 2500mAh；第三方实测 0.2C 时 3.0V = 90%、1C 时 3.0V = 79.5% 的 2.5V 容量）。因此放到 3.0V 的电芯，换算满容量会比实测**明显更大**，而非接近相等。

...
const DISCHARGE_CURVE = {
  ncm: [
    // [电压, 累计已放出容量比例 0~1] —— 依据 0.2C 标准放电实测曲线校正
    // 关键校正：4.2V→3.0V 仅放出约 90% 的满容量(至 2.5V)，
    // 3.0V→2.5V 尾段约占 8%~10%(大厂手册+Samsung INR25R 0.2C/2.5V 标称 2500mAh、
    // XTAR 实测 0.2C 时 3.0V=90% / 1C 时 3.0V=79.5% 的 2.5V 容量)，而非早前误设的 1%。
    [4.20, 0.00], [4.10, 0.06], [4.00, 0.13], [3.90, 0.22],
    [3.80, 0.32], [3.70, 0.43], [3.60, 0.54], [3.50, 0.64],
    [3.40, 0.73], [3.30, 0.81], [3.20, 0.86], [3.10, 0.885],
    [3.00, 0.90], [2.90, 0.935], [2.75, 0.975], [2.50, 1.00]
  ],
  lfp: [
    // LFP 平台极平(3.2~3.3V)，3.0V 以下仍有约 8%~10% 尾段
    [3.65, 0.00], [3.45, 0.06], [3.35, 0.22], [3.30, 0.45],
    [3.25, 0.65], [3.20, 0.80], [3.10, 0.88], [3.00, 0.92],
    [2.80, 0.98], [2.50, 1.00]
  ],
  lco: [
  ...
  custom: [
    [4.20, 0.00], [4.00, 0.13], [3.80, 0.32], [3.60, 0.54],
    [3.40, 0.73], [3.20, 0.86], [3.00, 0.90], [2.75, 0.975], [2.50, 1.00]
  ]
};
  ...
  return `
    <div class="know-grid">
      <div class="know-card">
        ...
        <div class="tip">📌 <strong>容量测试截止电压（标准 vs 使用）</strong>：按 IEC 61960 / GB 31241，电芯<strong>额定容量</strong>以 <strong>0.2C（C5，5 小时率）</strong>恒流放至截止电压测得——数码/消费三元为 <strong>2.75V</strong>，车用动力三元为 <strong>2.5V</strong>（如 Samsung INR18650-25R 标称 2500mAh 即 0.2C→2.5V）。BMS/设备的 <strong>3.0V 过放保护</strong>仅是使用下限，<strong>不是</strong>容量测试截止点；4.2V→3.0V 仅放出约 90% 容量，比放到 2.5V 少 8%~10%。</div>
      </div>
